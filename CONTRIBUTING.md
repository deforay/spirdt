...
The bar this codebase is held to, and the review that enforces it. The visual and
copy rules the assessor app is built to are in [web/DESIGN.md](web/DESIGN.md).
Neither is published to the documentation site, which serves the people who
install, use or evaluate the platform rather than the people who change it.
...
Each is also a Composer script, which is how they are usually run:
...
The build is a gate rather than a formality here, because the app is built into
`public/` — the document root — and deployed as it stands. Stage it with the
source it came from; see [Deployment](docs/deployment.md).

## 5. Development helpers

Three scripts that belong to whoever changes the code rather than to whoever
runs a server. All three live in `bin/dev/`.
...
The brief itself is section 1 above, extracted from this file at run time rather
than duplicated in the script. Two copies of a checklist means the
...
Puts a few facilities and testing sites into an organisation, so a fresh
development database has something to assess without going through the registry
screens first.
