...
- **The language a report is read in.** The report renders in the reader's
  language, not the language the visit was conducted in. Producing it in the
  latter means recording the locale on the assessment. The certificate, when it
  exists, has the same question to answer.
