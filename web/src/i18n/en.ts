/**
 * English — the source catalogue.
 *
 * Every other language is typed against this one, so a key added here and
 * forgotten elsewhere fails the build rather than showing up blank on a
 * tablet in the field.
 *
 * Two things are deliberately not in here. Question text, section titles and
 * response criteria live in the instrument template, because they are the
 * document being applied and change with its version. Server error messages
 * are returned already-worded by the API; translating those means giving the
 * API a locale, which is a separate piece of work.
 */

export const name = 'English'

/** English, Portuguese and Spanish all break at exactly one. */
export const plural = (count: number): 'one' | 'other' => (count === 1 ? 'one' : 'other')

export const messages = {
    'action.add': 'Add',
    'action.remove': 'Remove',
    'action.dismiss': 'Dismiss',
    'action.cancel': 'Cancel',
    /*
     * Examples, not instructions.
     *
     * Every text box on every management form carries one, because a form
     * where some boxes have grey text in them and some do not reads as a form
     * where the empty ones are somehow different. What they show is the SHAPE
     * of the answer — a code that looks like a code, a phone number with a
     * country on it — which is the one thing a label above the box cannot say.
     *
     * Names and places are examples and stay as they are in every language.
     * Anything with a word in it is translated.
     */
    'eg.facilityName': 'Chilenje Health Centre',
    'eg.facilityCode': 'ZM-LSK-014',
    'eg.address': 'Plot 42, Great East Road',
    'eg.personName': 'Grace Phiri',
    'eg.phone': '+260 977 123 456',
    'eg.email': 'lab@example.org',
    'eg.placeName': 'Chilanga',
    'eg.placeLevel': 'District',
    'eg.siteName': 'Main laboratory',
    'eg.siteLocation': 'Ground floor, east wing',
    'eg.jobTitle': 'Laboratory manager',
    'eg.organizationName': 'Ministry of Health',
    'eg.organizationCode': 'zm-moh',
    'eg.timezone': 'Africa/Lusaka',
    'eg.countryCode': 'ZM',

    'signIn.subtitle': 'Sign in to start an assessment.',
    'signIn.tagline': 'The stepwise assessment of rapid diagnostic testing sites.',
    'signIn.panelNote': 'Scored the same way in every laboratory, so one site can be compared with another and with itself a year later.',
    'signIn.email': 'Email',
    'signIn.password': 'Password',
    'signIn.organization': 'Org code',
    'signIn.submit': 'Sign in',
    'signIn.submitting': 'Signing in',
    'signIn.failed': 'Could not sign in.',
    'signIn.showPassword': 'Show password',
    'signIn.hidePassword': 'Hide password',
    'signIn.organizationHint': 'Your address exists in more than one organisation. Enter the code you were given.',
    'signIn.onlineNote': 'Signing in needs a connection. After that, assessments work offline.',

    'noAccess.title': 'This account has no screens yet',
    'noAccess.body': 'Nobody has given it access to anything. Ask your administrator, who can grant it from the people screen.',

    'password.title': 'Choose a password',
    'password.why': 'Your account is using a password somebody else set. Replace it before you start.',
    'password.current': 'Current',
    'password.new': 'New',
    'password.confirm': 'Repeat',
    'password.save': 'Save password',
    'password.saving': 'Saving',
    'password.failed': 'The password could not be changed.',
    'password.tooShort': 'Use at least {count} characters.',
    'password.mismatch': 'The two do not match.',
    'password.signsOutOthers': 'This signs you out on every other device.',

    'sites.title': 'Testing sites',
    'sites.subtitle': 'Choose the site you are assessing.',
    'sites.search': 'Search',
    'sites.unfinished': 'Unfinished audits',
    'sites.startNew': 'Start a new audit',
    'sites.draftProgress': '{answered} of {total} answered',
    'sites.showAll': 'Show all sites ({count} more)',
    'sites.showMine': 'Show only my sites',
    'sites.assignedToColleague': 'Assigned to a colleague',
    'sites.unassigned': 'Not assigned',
    'sites.loading': 'Loading sites.',
    'sites.empty': 'No sites yet. An administrator has to add the testing sites you assess.',

    'sync.blocked': 'Needs attention',
    'sync.running': 'Syncing',
    'sync.pending': '{count} waiting',
    'sync.never': 'Not synced',
    'sync.synced': 'Synced',

    'storage.notWritable':
        'This device is not saving anything. Turn off private browsing, or use a different browser, before you start.',
    'storage.mayClear':
        'Your work is saved on this device as you go. Add the app to your home screen to keep it safest, and sync when you have a signal.',
    'storage.notPersisted':
        'Your work is saved on this device as you go. Sync when you have a signal so nothing depends on this browser alone.',
    'storage.almostFull': 'This device is almost out of storage. Free up space before you start.',
    'storage.saveFailed':
        'The last answer was not saved to this device. Do not close this page.',
    'storage.answerNotSaved': 'The answer was not saved to this device.',

    'response.Y': 'Yes',
    'response.P': 'Partial',
    'response.N': 'No',
    'response.NA': 'N/A',

    'question.comment': 'Comments (optional)',

    'question.addNote': 'Add a note',
    'photos.heading': 'Photographs',
    'photos.count': '{taken} of {total}',
    'photos.add': 'Take a photo',
    'photos.addFromLibrary': 'Add from library',
    'photos.addAny': 'Add photographs',
    'photos.working': 'Saving…',
    'photos.none': 'No photographs of this part of the audit yet.',
    'photos.captionPlaceholder': 'What is in the picture',
    'photos.captionLabel': 'What this photograph shows',
    'photos.untitled': 'Photograph taken during this audit',
    'photos.view': 'View this photograph',
    'photos.position': '{index} of {total}',
    'photos.previous': 'Previous photograph',
    'photos.next': 'Next photograph',
    'photos.close': 'Close',
    'photos.actualSize': 'Actual size',
    'photos.fitToScreen': 'Fit to screen',
    'photos.remove': 'Delete this photograph',
    'photos.full': 'That is {count} photographs, which is as many as this part of the audit takes.',
    'photos.someFull': 'Added {added}. This part of the audit takes {count} photographs, so the rest were not.',
    'photos.notAnImage': 'That file is not an image.',
    'photos.failed': 'The photograph could not be saved on this device.',
    'photos.sectionEmpty': 'No photographs.',
    'photos.reviewHeading': 'Photographs',
    'photos.reviewCount': {
        one: '{count} photograph',
        other: '{count} photographs',
    },
    'photos.reviewEmpty': 'No photographs were taken during this audit.',

    'question.guidance': 'What to look for',
    'question.guidanceHide': 'Hide',
    'question.guidanceMore': 'More',
    'question.guidanceLess': 'Less',
    'question.whyNotApplicable': 'Why does this not apply?',
    'question.describeGap': 'Describe the gap',
    'question.noteRequired': 'A note is required before you can submit.',
    'question.responseLabel': 'Response to question {code}',
    'question.noteLabel': 'Note for question {code}',

    'context.changesChecklist': 'Changes the checklist',
    'invalid.not_an_integer': 'Enter a whole number.',
    'invalid.below_min': 'Cannot be less than {min}.',
    'invalid.above_max': 'Cannot be more than {max}.',
    'invalid.not_a_date': 'Enter a real date.',
    'invalid.in_the_future': 'Cannot be a date in the future.',
    'context.choose': 'Choose one',
    'context.specify': 'Please specify',
    'context.add': 'Add {label}',
    'date.open': 'Choose a date',
    'date.today': 'Today',
    'date.clear': 'Clear',
    'date.previousMonth': 'Previous month',
    'date.nextMonth': 'Next month',
    'date.previousYear': 'Previous year',
    'date.nextYear': 'Next year',
    'time.open': 'Choose a time',
    'time.now': 'Now',
    'time.hour': 'Hour',
    'time.minute': 'Minute',

    'pathogens.empty': 'Add every rapid test performed at this site.',
    'pathogens.placeholder': 'Add a pathogen',
    'pathogens.repeatNote': {
        one: 'Section {number} repeats for each one. {count} question.',
        other: 'Section {number} repeats for each one. {count} questions.',
    },

    'setup.eyebrow': 'Audit · step one',
    'setup.title': 'Set up the audit',
    'setup.pathogensHeading': 'Tests performed here',
    'setup.locationHeading': 'Where this audit is',
    'setup.locationAccuracy': 'accurate to {metres} m',
    'setup.locationCoarse': 'That is a district, not a building. Try again outside if you can.',
    'setup.locationNone': 'No position recorded. Nothing here needs one.',
    'setup.locationCapture': 'Record the position',
    'setup.locationRetry': 'Record it again',
    'setup.locationWorking': 'Looking…',
    'setup.roundHeading': 'This audit',
    'setup.auditRound': 'Audit round',
    'setup.auditRoundPlaceholder': 'e.g. 1',
    'setup.auditRoundHint': 'A number, or a word like Baseline. Every audit belongs to one — it is how this visit is compared with the last.',
    'review.auditRound': 'Audit round',
    'report.auditRound': 'Audit round',
    'reports.round': 'Round',
    'setup.contextHeading': 'About the site',
    'setup.peopleHeading': 'Who was spoken to',
    'setup.continueTo': 'Continue to {section}',
    'setup.needRound': 'Say which round this audit belongs to.',
    'setup.needPathogen': 'Add at least one test performed at this site.',
    'setup.missingFields': {
        one: '{count} required field still to fill in.',
        other: '{count} required fields still to fill in.',
    },
    'setup.invalidFields': {
        one: '{count} answer needs correcting.',
        other: '{count} answers need correcting.',
    },

    'assess.exit': 'Save and exit',
    'checklist.loading': 'Loading',
    'checklist.currentVisit': 'Current audit',
    'checklist.runningScore': 'Running score',
    'checklist.progress2': 'Progress',
    'checklist.sectionOf': 'Section {number} of {total}',
    'checklist.answered': '{answered} of {total} answered',
    'checklist.sections': 'Sections',
    'checklist.pathogens': 'Pathogens',
    'checklist.sectionScore': 'Section score',
    'checklist.editSetup': 'Site details',
    'checklist.sectionDone': 'Complete',
    'checklist.review': 'Review',
    'checklist.answeredLabel': 'answered',
    'checklist.previousSection': 'Previous',
    'checklist.nextSection': 'Next',
    'checklist.nextPathogen': 'Next pathogen',
    'checklist.provisional': 'Provisional',
    'checklist.progress': '{answered} of {total} answered',

    'save.error': 'Not saved',
    'save.saving': 'Saving',
    'save.nothing': 'Nothing to save yet',
    'save.saved': 'Saved {time}',

    'score.notScorable': 'Not scorable',
    'score.level': 'Level {level}',

    'actions.heading': 'Corrective actions',
    'review.back': 'Back to checklist',
    'review.title': 'Review',
    'review.points': '{score} of {possible} points',
    'review.unanswered': 'Unanswered',
    'review.unansweredNote':
        'Unanswered questions score zero, so this figure rises as the assessment is completed.',
    'review.gaps': 'Gaps',
    'review.described': '{described} of {total} described',
    'review.gapPlaceholder': 'What is missing or not being done',
    'review.recommendationPlaceholder': 'Recommended action',
    'review.whoActs': 'Who acts on this',
    'review.responsiblePerson': 'Responsible person',
    'review.notDescribed': 'Not described yet',
    'review.noGaps': 'No gaps recorded. Every answer so far is a Yes or Not applicable.',
    'review.needsNote': 'Needs a note',
    'review.needsNoteHint': 'A Partial, No or N/A has to say why. The site cannot act on a gap nobody described.',
    'review.notesNeeded': {
        one: '{count} answer still needs a note.',
        other: '{count} answers still need a note.',
    },
    'review.overview': 'Sections',
    'review.outstanding': {
        one: '{count} left',
        other: '{count} left',
    },
    'review.noteCount': {
        one: '{count} note',
        other: '{count} notes',
    },
    'review.bySection': 'By section',
    'review.notApplicable': 'Not applicable',
    'review.saveDraft': 'Save draft',
    'review.submit': 'Submit assessment',
    'review.submitting': 'Submitting',
    'review.stillNeeded': {
        one: '{count} question still needs an answer.',
        other: '{count} questions still need an answer.',
    },

    'signature.heading': 'Signatures',
    'signature.canvasLabel': 'Sign here',
    'signature.sign': 'Sign',
    'signature.signAgain': 'Sign again',
    'signature.undo': 'Undo',
    'signature.clear': 'Clear',
    'signature.done': 'Done',
    'signature.unsigned': 'Not signed',
    'signature.signedAt': 'Signed {time}',
    'signature.pending': 'Waiting to upload',
    'signature.assessor': 'Assessor',
    'signature.secondAssessor': 'Second assessor',
    'signature.siteRepresentative': 'Site representative',
    'signature.namePlaceholder': 'Name, if a second assessor attended',
    'signature.noName': 'Name not recorded',
    'signature.siteNameHint': 'Fill in the interviewee’s name in Part A first.',
    'signature.optional': 'Signatures are not required to submit, and upload separately.',

    'submit.failed': 'The assessment could not be submitted.',
    'submit.noAssessment': 'There is no assessment to submit.',
    'submit.invalidAnswers': 'Some answers are not allowed for their question.',

    'responsibility.site': 'Site',
    'responsibility.facility': 'Facility',
    'responsibility.district': 'District',
    'responsibility.regional': 'Regional',
    'responsibility.national': 'National',

    'locale.label': 'Language',
    'locale.instrumentNote':
        'The checklist itself stays in its published language until the instrument is translated.',

    'account.changePassword': 'Change password',
    'account.title': 'My account',
    'account.subtitle': 'Your details, this device, and your password.',
    'account.profile': 'Profile',
    'account.name': 'Name',
    'account.preferences': 'Preferences',
    'account.security': 'Security',
    'account.themeHint': 'Applies to this device only.',
    'account.languageHint': 'The language of the application, not of the instrument.',
    'admin.users': 'People',
    'admin.usersSubtitle': 'Who can sign in, and as what.',
    'admin.signOut': 'Sign out',
    'nav.registry': 'Registry',
    'nav.access': 'Access',
    'nav.menu': 'Menu',
    'nav.home': 'Home',
    'theme.title': 'Theme',
    'theme.light': 'Light',
    'theme.dark': 'Dark',
    'theme.auto': 'Auto',
    'nav.breadcrumb': 'Breadcrumb',
    'admin.addUser': 'Add someone',
    'admin.fullName': 'Full name',
    'admin.email': 'Email address',
    'admin.person': 'Person',
    'admin.role': 'Role',
    'admin.lastSignIn': 'Last signed in',
    'admin.actions': 'Actions',
    'admin.never': 'Never',
    'admin.loading': 'Loading.',
    'admin.loadFailed': 'Could not load the list.',
    'admin.actionFailed': 'That did not work.',
    'admin.resetPassword': 'Reset password',
    'admin.deactivate': 'Deactivate',
    'admin.activate': 'Activate',
    'admin.status': 'Status',
    'admin.active': 'Active',
    'admin.disabled': 'Disabled',
    'admin.mustChangePassword': 'Must choose a new password',
    'admin.passwordFor': 'Password for {name}',
    'admin.passwordOnce': 'Shown once. Hand it over in person, not by email.',
    'admin.passwordNoted': 'I have written it down',
    'role.admin': 'Administrator',
    'role.assessor': 'Assessor',
    'role.viewer': 'Viewer',
    'role.site_user': 'Site staff',
    'role.superadmin': 'Superadmin',

    'roles.title': 'Roles',
    'roles.subtitle': 'What each role in this organisation may do.',
    'roles.people': 'people',
    'roles.nobody': 'nobody',
    'roles.save': 'Save changes',
    'roles.saving': 'Saving',
    'roles.saved': 'Saved.',
    'roles.noChanges': 'Nothing has changed yet.',
    'roles.notEditable': 'This role is above yours, so what it may do is not yours to change.',
    'roles.cannotGrant': 'You do not have this yourself, so you cannot give it to anybody.',
    'roles.ownLock': 'You cannot take this off your own role. Nobody could put it back.',
    'perm.assessments.submit': 'File assessments',
    'perm.registry.read': 'See the registry',
    'perm.registry.write': 'Change the registry',
    'perm.assignments.write': 'Plan who covers what',
    'perm.reports.read': 'Read reports',
    'perm.reports.send': 'Email reports out',
    'perm.users.manage': 'Manage people',
    'perm.roles.manage': 'Manage roles',
    'perm.organizations.manage': 'Manage organisations',

    'audit.title': 'Activity',
    'audit.subtitle': 'What has been done in this organisation, and by whom.',
    'audit.when': 'When',
    'audit.who': 'Who',
    'audit.what': 'What',
    'audit.details': 'Details',
    'audit.allActions': 'Everything',
    'audit.from': 'From',
    'audit.to': 'To',
    'audit.none': 'Nothing matches those filters.',
    'audit.system': 'The system',
    'audit.deletedActor': 'Account {id}, since removed',
    'audit.showing': 'Showing {shown} of {total}',
    'audit.more': 'Show more',
    'audit.session': 'Session',
    'audit.clear': 'Clear filters',
    'requests.title': 'Requests',
    'requests.subtitle': 'What this server has been asked, and what it answered.',
    'requests.outcome': 'Outcome',
    'requests.allOutcomes': 'Everything',
    'requests.failed': 'Anything that failed',
    'requests.refused': 'Refused (4xx)',
    'requests.broke': 'Server error (5xx)',
    'requests.method': 'Method',
    'requests.allMethods': 'Any method',
    'requests.path': 'Path',
    'requests.request': 'Request',
    'requests.answer': 'Answer',
    'requests.took': 'Took',
    'requests.none': 'Nothing matches those filters.',
    'requests.unauthenticated': 'Not signed in yet',
    'requests.followingSession': 'Following one session, including the requests it made before signing in.',
    'requests.uid': 'Log reference',
    'requests.origin': 'Came from',
    'requests.device': 'Device',
    'requests.appVersion': 'App version',
    'requests.body': 'What was sent',
    'action.auth.signed_in': 'Signed in',
    'action.auth.signed_out': 'Signed out',
    'action.auth.password_changed': 'Changed their password',
    'action.auth.token_replayed': 'Session ended after a token was reused',
    'action.user.created': 'Added an account',
    'action.user.updated': 'Changed an account',
    'action.user.password_reset': 'Reset an account password',
    'action.role.permissions_changed': 'Changed what a role may do',
    'action.organization.created': 'Added an organisation',
    'action.organization.updated': 'Changed an organisation',
    'action.assessment.submitted': 'Submitted an assessment',
    'action.report.sent': 'Emailed a report to the site',
    'action.report.send_failed': 'Tried to email a report and it did not go',
    'action.facility.merged': 'Merged two facilities',

    'dash.eyebrow': 'Overview',
    'dash.title': 'Dashboard',
    'dash.subtitle': 'Where this country stands, from the audits submitted so far.',
    'dash.certifiable': 'Sites eligible for certification',
    'dash.certifiableHelp': 'Share of scored assessments at Level {level} or above.',
    'dash.ofScored': '{count} of {total} scored',
    'dash.assessments': 'Assessments submitted',
    'dash.sites': 'Testing sites assessed',
    'dash.facilities': 'Facilities covered',
    'dash.drafts': 'Started, not submitted',
    'dash.ofRegistry': 'of {total} in the registry',
    'dash.levels': 'Certification levels',
    'dash.levelsRecent': 'Last 30 days',
    'dash.overTime': 'Assessments by month',
    'dash.sections': 'Weakest sections',
    'dash.sectionsHelp': 'Mean score across every submitted assessment, weakest first.',
    'dash.latest': 'Latest assessments',
    'dash.nothing': 'No assessment has been submitted yet. This fills in as audits arrive.',
    'dash.nothingRecent': 'Nothing in the last 30 days.',
    'dash.noSections': 'Section scores appear once an assessment has been submitted.',
    'dash.visits': '{count} of {total}',
    'dash.mean': 'mean {value}%',
    'dash.viewAll': 'All assessments',

    'dash.viewData': 'View data',
    'dash.fromDate': 'From',
    'dash.toDate': 'To',
    'dash.auditCount': 'Assessments',
    'dash.beginning': 'The beginning',
    'dash.today': 'Today',
    'dash.saveImage': 'Save image',
    'dash.range': 'Period',
    'dash.rangeAll': 'All time',
    'dash.range30': 'Last 30 days',
    'dash.range90': 'Last 90 days',
    'dash.range180': 'Last 180 days',
    'dash.range365': 'Last 12 months',
    'dash.rangeCustom': 'Custom',
    'dash.place': 'Place',
    'dash.everywhere': 'Everywhere',
    'dash.filtered': 'Filtered',
    'dash.clear': 'Clear',
    'dash.allTime': 'All time',
    'dash.trend': 'Last 180 days',
    'dash.radar': 'Section profile',
    'dash.radarHelp': 'Mean score per section. A dent is where a country is losing points.',
    'dash.radarTooFew': "A profile needs at least three sections with a score. There are {count} so far.",
    'dash.map': 'Where audits happened',
    'dash.mapEmpty': 'No assessment carries a position yet. Coordinates are recorded when an assessor files an audit, and fall back to the facility when the registry has one.',
    'dash.showThese': 'Show these',
    'map.inherited': 'Position from the facility record, not from the audit',
    'map.device': 'Recorded on the audit',
    'map.facility': 'From the facility record',

    'registry.title': 'Registry',
    'registry.subtitle': 'Places, facilities, and the testing sites inside them.',
    'registry.place': 'Place',
    'registry.all': 'All',
    'registry.clear': 'Clear',
    'registry.placesWithin': 'Places within',
    'registry.noPlaces': 'Nothing below this yet.',
    'registry.placeName': 'Name',
    'registry.levelName': 'Level',
    'registry.facilities': 'Facilities',
    'registry.noFacilities': 'No facilities here yet.',
    'registry.facilityName': 'Facility name',
    'registry.testingSites': 'Testing sites',
    'registry.noSites': 'No testing sites here yet.',
    'registry.siteName': 'Testing site name',
    'registry.chooseFacility': 'Choose a facility to see its testing sites.',
    'registry.fromTheField': 'added in the field',
    'assignments.title': 'Assignments',
    'assignments.subtitle': 'Who covers which site.',
    'assignments.assignTo': 'Assign to',
    'assignments.wholeOrganisation': 'Anyone in the organisation',
    'assignments.coveredBy': 'Covered by',
    'assignments.assign': 'Assign',
    'assignments.withdraw': 'Withdraw',
    'assignments.noSites': 'No testing sites in this selection.',

    'review.gapNumber': 'Gap {number}',
    'review.howUrgent': 'How urgent',
    'review.describeThisGap': 'Describe this gap',
    'review.addAnotherGap': 'Add another gap',
    'urgency.immediate': 'Immediate',
    'urgency.followUp': 'Follow-up',

    'organizations.title': 'Country',
    'organizations.subtitle': 'The organisations auditing here.',
    'organizations.add': 'Add an organisation',
    'organizations.organisation': 'Organisation',
    'organizations.name': 'Name',
    'organizations.code': 'Short code',
    'organizations.adminName': 'Administrator\'s name',
    'organizations.adminEmail': 'Administrator\'s email',
    'organizations.createsAdmin': 'Creates the organisation and its first administrator together, and shows their password once.',
    'organizations.created': '{name} created',
    'organizations.assessments': 'Assessments',
    'organizations.noAdmin': 'nobody can administer it',

    'places.title': 'Geography',
    'places.subtitle': 'Provinces, districts, and whatever else this country uses.',
    'places.search': 'Search places',
    'places.addingUnder': 'Adding under: {place}',
    'places.topLevel': 'top level',
    'places.moveToTop': 'move to top level',
    'places.addUnder': 'Add under',
    'facilities.title': 'Facilities',
    'facilities.subtitle': 'Hospitals, clinics and health centres.',
    'facilities.search': 'Search by name or code',
    'facilities.anywhere': 'Anywhere',
    'facilities.add': 'Add a facility',
    'facilities.noPlace': 'No place set',
    'sitesAdmin.title': 'Testing sites',
    'sitesAdmin.subtitle': 'The benches an assessment is made against.',
    'sitesAdmin.search': 'Search testing sites',
    'sitesAdmin.add': 'Add a testing site',
    'sitesAdmin.noFacilitiesYet': 'No facilities yet — add one first',
    'sitesAdmin.noFacilityMatch': 'No facility of that name.',
    'sitesAdmin.findFacility': 'Find the facility',
    'registry.change': 'Change',
    'registry.searchPlaces': 'Search places',
    'placePicker.add': 'Add a place',
    'placePicker.addTitle': 'New place',
    'placePicker.addAction': 'Add it',
    'placePicker.inside': 'Inside',
    'placePicker.levelExample': 'District, province, region',
    'placePicker.required': 'A place needs a name and a level.',
    'registry.noPlacesFound': 'No places found. Add them under Geography.',
    'registry.nothingFound': 'Nothing found.',
    'registry.showing': '{from}–{to} of {total}',
    'registry.pages': 'Pages',
    'registry.previous': 'Previous',
    'registry.next': 'Next',
    'registry.pageOf': '{page} of {pages}',
    'assignments.selected': '{count} selected',
    'assignments.assignSelected': 'Assign all selected',
    'assignments.selectAll': 'Select everything on this page',

    'form.back': 'Back',
    'form.save': 'Save',
    'form.saving': 'Saving',
    'flash.added': '{name} added.',
    'flash.saved': '{name} saved.',
    'flash.merged': '{name} merged into {into}.',
    'flash.activated': '{name} activated.',
    'flash.deactivated': '{name} deactivated.',
    'facilityForm.addTitle': 'New facility',
    'facilityForm.addSubtitle': 'A hospital, clinic or health centre.',
    'facilityForm.editSubtitle': 'Everything the registry knows about this facility.',
    'facilityForm.name': 'Facility name',
    'facilityForm.code': 'Facility code',
    'facilityForm.codeHint': 'However this facility is identified in national records.',
    'facilityForm.place': 'Where it is',
    'facilityForm.placeHint': 'Type a district or province name — the list narrows as you type.',
    'facilityForm.sectionNames': 'What it is called',
    'facilityForm.sectionWhere': 'Where it is',
    'facilityForm.sectionKind': 'What kind of facility',
    'facilityForm.sectionContact': 'Who to contact',
    'facilityForm.type': 'Type of facility',
    'facilityForm.level': 'Level',
    'facilityForm.affiliation': 'Affiliation',
    'facilityForm.unset': 'Not set',
    'facilityForm.address': 'Address',
    'facilityForm.contactName': 'Contact person',
    'facilityForm.contactHint': 'Who to reach at the facility itself, not the person interviewed on an audit.',
    'facilityForm.contactPhone': 'Phone',
    'facilityForm.contactEmail': 'Email',
    'facilityForm.coordinates': 'Coordinates',
    'facilityForm.coordinatesHint': 'Latitude then longitude, in decimal degrees.',
    'facilityForm.latitude': 'Latitude',
    'facilityForm.longitude': 'Longitude',
    'facilityForm.nameRequired': 'A facility name is required.',
    'facilityForm.merge': 'Merge into another',
    'facilityForm.mergeTitle': 'Merge this facility',
    'facilityForm.mergeExplain': 'This facility is folded into the one you choose. Its testing sites move across, and nothing is deleted — assessments already made against it keep resolving.',
    'facilityForm.mergeFind': 'Find the facility to keep',

    'siteForm.addTitle': 'New testing site',
    'siteForm.subtitle': 'A bench, room or counter where testing happens.',
    'siteForm.name': 'Testing site name',
    'siteForm.nameHint': 'What people at the facility call it — "TB clinic", "ART corner".',
    'siteForm.facility': 'Facility it belongs to',
    'siteForm.location': 'Where in the facility',
    'siteForm.locationHint': 'Optional. Helps an assessor find it on arrival.',
    'siteForm.required': 'A name and a facility are required.',
    'siteForm.visits': 'Where audits of this site happened',
    'siteForm.visitsEmpty': 'No audit of this site carries a position. Coordinates are recorded when an assessor files one, and fall back to the facility when the registry has them.',
    'siteForm.visitsHint': 'One pin per submitted audit. Choose one to open its report.',
    'siteForm.accuracy': 'accurate to {metres} m',

    'sitePhoto.label': 'Photograph of the site',
    'sitePhoto.hint': 'Optional. What the bench looks like, so an assessor arriving knows they are at the right one.',
    'sitePhoto.none': 'No photograph yet.',
    'sitePhoto.saveFirst': 'Save the site first, then add a photograph.',
    'sitePhoto.take': 'Take or choose a photo',
    'sitePhoto.replace': 'Replace photo',
    'sitePhoto.remove': 'Remove',
    'sitePhoto.removeConfirm': 'Delete this photograph?',
    'sitePhoto.removeYes': 'Delete',
    'sitePhoto.working': 'Sending…',
    'sitePhoto.alt': 'Photograph of {name}',
    'sitePhoto.notAnImage': 'That file is not an image.',
    'placeForm.addTitle': 'New place',
    'placeForm.subtitle': 'One level of the country\'s hierarchy.',
    'placeForm.name': 'Name',
    'placeForm.level': 'Level',
    'placeForm.levelHint': 'What this tier is called here — Province, Region, Woreda.',
    'placeForm.parent': 'Sits within',
    'placeForm.parentHint': 'Leave empty for a top-level place.',
    'placeForm.topLevel': 'Top level',
    'placeForm.required': 'A name and a level are required.',
    'userForm.addTitle': 'New person',
    'userForm.subtitle': 'Who they are and what they may do.',
    'userForm.required': 'A name and an email address are required.',
    'userForm.emailFixed': 'The sign-in address cannot be changed.',
    'userForm.ownRole': 'You cannot change your own role.',
    'userForm.passwordIs': 'Their password',
    'userForm.title': 'Job title',
    'userForm.phone': 'Phone',
    'organizationForm.addTitle': 'New organisation',
    'organizationForm.subtitle': 'An organisation auditing in this country.',
    'organizationForm.codeHint': 'Short and lowercase. Typed at sign-in where one address exists in two organisations.',
    'organizationForm.codeFixed': 'The code cannot be changed once people have learned it.',
    'organizationForm.timezone': 'Timezone',
    'organizationForm.timezoneHint': 'An IANA name, such as Africa/Lusaka.',
    'organizationForm.country': 'Country code',
    'organizationForm.adminHint': 'Created with the organisation, so it is never left with nobody able to administer it.',
    'organizationForm.adminIs': 'First administrator',
    'reports.title': 'Reports',
    'reports.subtitle': 'Every audit, newest first.',
    'reports.search': 'Search a site or facility',
    'reports.anyStatus': 'Any status',
    'reports.anyLevel': 'Any level',
    'reports.statusDraft': 'Draft',
    'reports.statusSubmitted': 'Submitted',
    'reports.statusReviewed': 'Reviewed',
    'reports.statusFinalised': 'Finalised',
    'reports.from': 'From',
    'reports.to': 'To',
    'reports.nothingYet': 'No audits have been collected yet.',
    'reports.notScoredYet': 'Not scored yet',
    'reports.siteGone': 'Site no longer listed',
    'report.title': 'Assessment report',
    'report.print': 'Print',
    'report.pdf': 'PDF',
    'report.send': 'Send to the site',
    'report.sending': 'Sending…',
    'report.sendHint': 'The report goes to the laboratory as an email with the document attached.',
    'report.sendTo': 'Email address',
    'report.sendToPlaceholder': 'eg. laboratory@hospital.org',
    'report.sendToUnknown': 'The registry holds no contact for this facility. What you enter here is kept for next time.',
    'report.sendToUnknownReadOnly': 'The registry holds no contact for this facility. Your role cannot add one, so this address is used for this send only.',
    'report.sendWhat': 'What to send',
    'report.sendDone': 'Sent to {email}.',
    'report.sendFailed': 'The report could not be sent.',
    'report.sendHistory': 'Already sent',
    'report.sentTo': 'Sent to {email}',
    'report.sendFailedTo': 'Could not be sent to {email}',
    'report.downloadPdf': 'Download PDF',
    'report.pdfWithPhotographs': 'Full report with photographs',
    'report.pdfWithoutPhotographs': 'Full report without photographs',
    'report.pdfActions': 'Corrective actions only',
    'report.pdfPreparing': 'Preparing…',
    'report.pdfFailed': 'The report could not be downloaded.',
    'report.record': 'Assessment record',
    'report.place': 'Place',
    'report.history': 'Audit history',
    'report.started': 'Assessment started',
    'report.ended': 'Assessment ended',
    'report.submittedAt': 'Submitted',
    'report.scoredAt': 'Scored',
    'report.signedBy': 'Signed by {name}',
    'report.facilityCode': 'Facility code',
    'report.assessedOn': 'Assessed on',
    'report.previousVisit': 'Previous audit',
    'report.pathogens': 'Pathogens',
    'report.status': 'Status',
    'report.outOf': '{score} of {possible} points',
    'report.section': 'Section',
    'report.scored': 'Scored',
    'report.excluded': 'Not applicable',
    'report.percent': 'Percentage',
    'report.anomalies': 'Some answers were recorded that the instrument did not expect. They are kept with the score.',
    'report.actionPlan': 'Corrective action plan',
    'report.immediate': 'Immediate',
    'report.followUp': 'Follow-up',
    'report.unanswered': 'Not answered',
    'report.photographs': 'Photographs',
    'report.imageUnavailable': 'This image could not be loaded.',
    'report.sitePhotographs': 'Photographs of the site',
    'report.signatures': 'Signatures',
    'form.edit': 'Edit',

    'report.overall': 'Overall',

    'report.level': 'Level',

    'report.sections': 'Sections',

    'report.findings': 'Findings',
    'report.bySection': 'By section',
    'report.excludedCount': '{count} not applicable',
    'report.due': 'Due',
    'report.draftWarning': 'Draft — these figures are not final',

    'noAccess.contact': 'Ask {name}, at {email}.',
    'noAccess.contactEmail': 'Ask {email}.',

    'settings.title': 'Settings',
    'settings.subtitle': 'How this installation is set up.',
    'settings.instance': 'This installation',
    'settings.instanceHint': 'Shown in the header, and to anybody who cannot get in.',
    'settings.name': 'Name',
    'settings.nameHint': 'Replaces SPI-RDT in the header. Leave it empty to keep that.',
    'settings.contactName': 'Who to contact',
    'settings.contactEmail': 'Contact email',
    'settings.contactEmailHint': 'Offered to an account that has been given no access yet.',
    'settings.localisation': 'Localisation',
    'settings.localisationHint': 'For {organization}. Every organisation has its own.',
    'settings.timezone': 'Timezone',
    'settings.timezoneHint': 'Decides where a day begins when activity is filtered by date.',
    'settings.language': 'Language',
    'settings.languageHint': 'What a device starts in until somebody switches it.',
    'settings.country': 'Country',
    'settings.countryHint': 'Two letters, as in ZM or KE.',
    'settings.mail': 'Email delivery',
    'settings.mailHint':
        'Kept for the messages this platform does not send yet. Nothing reads it until something does.',
    'settings.host': 'Server',
    'settings.port': 'Port',
    'settings.encryption': 'Encryption',
    'settings.none': 'None',
    'settings.username': 'Username',
    'settings.password': 'Password',
    'settings.passwordSet': 'A password is stored. Leave this empty to keep it.',
    'settings.passwordUnset': 'No password is stored.',
    'settings.passwordClear': 'Remove the stored password',
    'settings.noKey':
        'This server has no APP_KEY, so a password cannot be stored safely. Everything else here saves as usual.',
    'settings.fromAddress': 'From address',
    'settings.fromName': 'From name',
    'settings.saved': 'Saved.',
}
